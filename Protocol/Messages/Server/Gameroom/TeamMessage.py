...
class TeamMessage(Writer):
    ...
    def encode(self):
        ...
        for x in range(1):
            ...
            self.writeDataReference(29, self.player.brawlers_skins[str(self.player.homeBrawler)])

            self.writeVint(self.player.brawlers_trophies[str(self.player.homeBrawler)])
            self.writeVint(self.player.brawlers_high_trophies[str(self.player.homeBrawler)])
            self.writeVint(10)
